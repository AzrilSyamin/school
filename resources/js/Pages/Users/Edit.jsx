...
export default function Edit({ user, roles, classrooms }) {
    ...
    const isSelfModerator = auth.user.role === 'moderator' && auth.user.id === user.id;

    return (
        <AuthenticatedLayout title="Kemaskini Pengguna">
            ...
            <div className="max-w-4xl space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Kemaskini Maklumat Pengguna</h1>
                        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Edit maklumat akaun dan peranan pengguna.</p>
                    </div>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 dark:border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6">
                    <form onSubmit={submit} className="space-y-8">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full text-sm text-slate-500 dark:text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-indigo-600 file:text-white hover:file:bg-indigo-500 cursor-pointer"
                                    />
                        ...
                        <div className="space-y-6 pt-6 border-t border-slate-200 dark:border-slate-200 dark:border-slate-800">
                            <h2 className="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                                <span className="w-1.5 h-1.5 rounded-full bg-indigo-500" />
                                Tetapan Akaun & Peranan
                            </h2>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    <label htmlFor="role_id" className={labelClass}>Peranan (Role)</label>
                                    <select
                                        id="role_id"
                                        value={data.role_id}
                                        onChange={(e) => setData('role_id', e.target.value)}
                                        className={`${inputClass} appearance-none ${isSelfModerator ? 'opacity-50 cursor-not-allowed bg-white dark:bg-slate-900' : ''}`}
                                        disabled={isSelfModerator}
                                    >
                                    ...
                                    <InputError message={errors.role_id} className="mt-1.5 text-xs text-red-400" />
                                </div>
                                <div>
                                    <label htmlFor="classroom_id" className={labelClass}>Kelas (Hanya untuk Classrep / Pelajar)</label>
                                    <select
                                        id="classroom_id"
                                        ...
                                        className={`${inputClass} appearance-none`}
                                    >
                                        <option value="">-- Tiada Kelas / Tidak Berkenaan --</option>
                                ...
                                <div>
                                    <label htmlFor="is_active" className={labelClass}>Status Akaun</label>
                                    <select
                                        id="is_active"
                                    ...
                                    <InputError message={errors.is_active} className="mt-1.5 text-xs text-red-400" />
                                </div>
                                <div>
                        ...
                        <div className="pt-6 border-t border-slate-200 dark:border-slate-200 dark:border-slate-800 flex items-center justify-end gap-3">
                            ...
                            <button
                                type="submit"
                                disabled={processing}
                                className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-sm font-semibold text-white transition-colors shadow-lg shadow-indigo-500/20 disabled:opacity-50"
                            >
                                {processing && (
